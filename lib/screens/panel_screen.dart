...
class _AccessTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListView(
      padding: const EdgeInsets.all(16),
      children: [
        _AccessRow(role: 'Официант', permissions: const ['Создание заказов', 'Закрытие стола', 'Чаты']),
        const SizedBox(height: 12),
        _AccessRow(role: 'Повар / Бармен', permissions: const ['Просмотр заказов', 'Отметка готовности', 'Чаты']),
        const SizedBox(height: 12),
        _AccessRow(role: 'Менеджер', permissions: const ['Все функции', 'Отчеты', 'Управление столами']),
      ],
    );
  }
}
